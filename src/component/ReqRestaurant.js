...
export default function ReqRestaurant(){

    const {restaurantList} = useContext(MyContext);
    console.log("hhhhhhhhhhhh"+restaurantList);
        ...
        <div className="container-fluid mt-3 ">
            <div id="rest-list-container">
                <table className="table table-hover ">
                    ...
                    { restaurantList.sort((a, b) => a.name.localeCompare(b.name)).filter((restActive)=> restActive.status.toUpperCase() == 'PENDING').map((res, index) => <tr id={"row"+index} key={index}>
                            <td>{index+1}</td>
                            <td>{res.name}</td>
